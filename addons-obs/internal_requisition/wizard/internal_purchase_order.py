                ...
                for supp, val in po_lines.iteritems():
                    if len(val):
                        vals = {
                            ...
                            'payment_term_id': supp.property_supplier_payment_term.id
                        }

                        purchase_order = po_obj.create(vals)
                        pdb.set_trace()

                        template = self.pool.get('ir.model.data').get_object_reference(self.env.cr,
                                                                                  self.env.uid,
                                                                                  'purchase',
                                                                                  'email_template_edi_purchase')[1]
                        self.pool['email.template'].send_mail(self.env.cr,
                                                                     self.env.uid, template,
                                                                     purchase_order.id, force_send=True,
