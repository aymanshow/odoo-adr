...
class obs_small_label_template(models.AbstractModel):
    ...
    def get_barcode(self, value, width, barWidth = 0.05 * units.inch, fontSize = 8, humanReadable = True):
        ...
        barcode_str = '<img style="width:320;height:30px;"  src="data:image/png;base64,{0} : ">'.format(barcode_encode)
        return barcode_str
            ...
            for product in product_obj.browse(prod.product_id.id):
                ...
                for qty in range(int(prod.qty)):
                    ...
                    product_data.update({'ean13' : barcode_str, 'list_price' : list_price, 'display_price' : data['form']['display_price'],
                                        # 'name' : str(product.name)[:90], 'default_code': product.default_code})
