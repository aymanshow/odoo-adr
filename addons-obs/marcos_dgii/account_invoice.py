...
class account_invoice(orm.Model):
    ...
    def invoice_validate(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            partner_id.append(record.partner_id.id)

        cr.execute("SELECT has_rnc FROM account_journal WHERE name = '"+name+"';")
        res = cr.fetchmany()
        cr.execute("SELECT ref FROM res_partner WHERE id = {}".format(partner_id[0]))
        id = cr.fetchmany()

        if res[0][0] == None:
            super(account_invoice, self).write(cr, uid, ids, {'state':'open'}, context=context)
            return True
        elif res[0][0] != None and id[0][0] == None:
            super(account_invoice, self).write(cr, uid, ids, {'state':'draft'}, context=context)
            raise orm.except_orm(_('Advertencia'), _("No puede utilizar este diario para realizar esta factura.\n"
                                                         "El cliente no posee RNC/CEDULA!"))
            return False
        else:
            try:
                validar_rnc = rnc_parser.get_rnc_record(int(id[0][0]))
                super(account_invoice, self).write(cr, uid, ids, {'state':'open'}, context=context)
                return True
            except Exception:
                raise orm.except_orm(_('Advertencia'), _("No puede utilizar este diario para realizar esta factura.\n"
                                                         "Este RNC o Cédula no esta registrado como contribuyente en la DGII"))
                return False
    ...
    def _get_journal_id(self, fiscal_type, refund):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context.get('active_model', False) == 'pos.order' and vals.get('type', False) in ["out_invoice", 'out_refund']:
            ...
            #vals['journal_id'] = self._get_journal_id(fiscal_type, False)
            ...
            if vals.get('origin', False):
                ...
                vals['journal_id'] = self._get_journal_id(None, True)
            else:
                vals['reference'] = u""
