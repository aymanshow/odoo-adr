...
class purchase_order(osv.osv):
    ...
    STATE_SELECTION = [
        ...
        ('supplier_verified', 'Esperando Confirmacion Suplidor'),
    ...
    READONLY_STATES = {
        ...
        'supplier_verified': [('readonly', True)],
        'done': [('readonly', True)]
    }
    ...
    _track = {
        'state': {
            ...
            'purchase.mt_rfq_supplier_verified': lambda self, cr, uid, obj, ctx=None: obj.state == 'supplier_verified',
        },
    }
    _columns = {
    ...
    def check_company_purchase(self, cr, uid, ids, context=None):
        """
        @return: True or False
        """
        res = False
        order = self.browse(cr, uid, ids[0], context=context)
        company_rec = order.company_id.id
        if company_rec == 1:
            res = True
        return res
    ...
    def wkf_not_main_confirm_order(self, cr, uid, ids, context=None):
        todo = []
        for po in self.browse(cr, uid, ids, context=context):
            if not any(line.state != 'cancel' for line in po.order_line):
                raise osv.except_osv(_('Error!'),_('You cannot confirm a purchase order without any purchase order line.'))
    ...
    def action_last_approve(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids, {'state': 'supplier_verified'}, context=context)
        return True

    def action_supplier_verified(self, cr, uid, ids, context=None):
