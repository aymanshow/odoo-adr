...
class Settlement(models.Model):
    ...
    professional = fields.Many2one(
        comodel_name="res.partner", domain="[('professional', '=', True)]")
    professional_type = fields.Selection(related='professional.professional_type')
    ...
    def _prepare_invoice_header(self, settlement, journal, date=False):
        ...
        invoice_vals = {
            'partner_id': settlement.professional.id,
...
class SettlementLine(models.Model):
    ...
    professional_line = fields.Many2many(
        ...
        relation='settlement_professional_line_rel', column1='settlement_id',
        column2='professional_line_id', required=True)
    date = fields.Date(related="professional_line.invoice_date", store=True)
    invoice_line = fields.Many2one(
        comodel_name='account.invoice.line', store=True,
        related='professional_line.invoice_line')
    invoice = fields.Many2one(
        comodel_name='account.invoice', store=True, string="Invoice",
        related='invoice_line.invoice_id')
    professional = fields.Many2one(
        comodel_name="res.partner", readonly=True, related="professional_line.professional",
        store=True)
    settled_amount = fields.Float(
        related="professional_line.amount", readonly=True, store=True)
    commission = fields.Many2one(
        comodel_name="sale.commission", related="professional_line.commission")
