...
class TestContractCommission(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.commission = self.env['sale.commission'].create(
            {'name': 'Test',
             ...
             'fix_qty': 3.0})
        self.professional = self.env['res.partner'].create(
            {'name': 'Professional',
             'professional': True,
        ...
        self.partner.professionals = [(6, 0, self.professional.ids)]
    ...
    def test_invoice_commissions(self):
        ...
        self.assertEqual(len(invoice_lines.professionals), 1)
        self.assertEqual(invoice_lines.professionals.professional, self.professional)
        self.assertEqual(
            invoice_lines.professionals.commission, self.commission)
