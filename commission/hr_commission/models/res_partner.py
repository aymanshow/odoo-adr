...
class ResPartner(models.Model):
    ...
    def _get_employee(self):
        ...
        self.employee = False
        employee = self.env['hr.employee'].search([('address_home_id', '=', self.id)])
        if employee:
    ...
    @api.constrains('professional_type', 'employee')
    def _check_employee(self):
        if self.professional_type == 'salesman' and not self.employee:
    ...
    def onchange_professional_type(self):
        if self.professional_type == 'salesman':
            self.supplier = False
        return super(ResPartner, self).onchange_professional_type()
